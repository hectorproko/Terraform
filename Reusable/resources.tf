...
# NETWORKING #
resource "aws_vpc" "vpc" {              #Using workspace to specify env
...
resource "aws_subnet" "subnet" {
  count                   = var.subnet_count[terraform.workspace] #using workspace 
...
# S3 Bucket config#
resource "aws_iam_role" "allow_nginx_s3" {#envs are in the same AWS account, they share same IAM, so each roles needs to ben named diff
